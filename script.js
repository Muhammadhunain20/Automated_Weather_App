...
const getweatherdata = async (location = "", lat = null, lon = null) => {
    ...
    if (lat && lon) {
        ...
        response = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${location}&appid=${apikey}`);
    } else {
...
const fetchlocationweather = async () => {
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(async (position) => {
            ...
            // If location access is denied, show a message or fallback
            displayweatherdata({});
        });
    } else {
        // If Geolocation is not supported, show a message or fallback
        displayweatherdata({});
    }
}
